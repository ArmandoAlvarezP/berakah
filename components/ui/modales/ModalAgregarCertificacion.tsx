...
import { agregarCertificacion } from "@/actions";
...
export const ModalAgregarCertificacion = ({alumnos}: Props) => {
    ...
    // Manejo de formulario 
        ...
        const onSubmit: SubmitHandler<Inputs> = async (data) => {
            ...
            setErrorMessage('');
            const { tipo, fechaEmision, especialidad, emisora, autorizada, folio } = data;
    
            // Server action 
    
            const resp = await agregarCertificacion(id, tipo, fechaEmision, especialidad, emisora, autorizada, folio);
        ...
        >
            <div role="alert" className="container mx-auto w-11/12 md:w-2/3 max-w-lg">
                <div className="relative py-8 px-5 md:px-10 bg-white shadow-md border border-gray-400 rounded-4xl">
                    ...
                    <h3 className="text-[#10D4B3] text-center font-semibold uppercase italic mb-5">{alumno[0].nombre}</h3>
                    <select 
                        id="tipo" 
                        className={
                            clsx(
                                "mb-5 mt-2 text-gray-400 focus:outline-none focus:border focus:border-[#00795D] w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded-3xl border uppercase",
                                {
